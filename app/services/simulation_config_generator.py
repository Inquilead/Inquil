...
## 任务
请生成事件配置JSON：
- 提取热点话题关键词（若涉及创业/产品，围绕「想法验证」：产品价值、商业模式、竞争、融资、用户痛点等）
- 描述舆论/讨论发展方向（各方如何质疑、支持或传播该想法）
- 设计初始帖子内容，**每个帖子必须指定 poster_type（发布者类型）**

**重要**: poster_type 必须从上面的"可用实体类型"中选择，这样初始帖子才能分配给合适的 Agent 发布。
例如：投资人观点由 VentureInvestor/AngelInvestor 等类型发布，创始人讨论由 StartupFounder，客户/用户反馈由 TargetCustomer/SkepticalUser，新闻由 MediaOutlet 发布。
...
{{
    ...
    "reasoning": "<简要说明>"
        ...
        system_prompt = (
            "你是舆论与创业想法验证场景分析专家。返回纯JSON格式。"
            "poster_type 必须精确匹配可用实体类型；初始讨论应能引出投资、产品、市场等多角度反应。"
        )
    ...
    def _assign_initial_post_agents(
        ...
        type_aliases = {
            ...
            "ventureinvestor": ["ventureinvestor", "angelinvestor", "acceleratorpartner", "person"],
            "angelinvestor": ["angelinvestor", "ventureinvestor", "person"],
            "acceleratorpartner": ["acceleratorpartner", "ventureinvestor", "person"],
            "startupfounder": ["startupfounder", "person"],
            "targetcustomer": ["targetcustomer", "skepticaluser", "person"],
            "skepticaluser": ["skepticaluser", "targetcustomer", "person"],
            "industryexpert": ["industryexpert", "expert", "person"],
            "startupcompany": ["startupcompany", "company", "organization"],
            "venturecapitalfirm": ["venturecapitalfirm", "company", "organization"],
        }
...
- **创业验证常见角色**：VC/天使（VentureInvestor/AngelInvestor 等）活跃度中、工作时间偏活跃、发言少但影响力高(2.0-3.0)；StartupFounder 活跃度高、晚间与午间活跃；TargetCustomer/SkepticalUser 活跃度高、响应快、影响力中等(0.9-1.3)
...
{{
    "agent_configs": [
        ...
        system_prompt = (
            "你是社交媒体行为分析专家，当前场景常为创业想法验证。返回纯JSON，配置需符合中国人作息习惯；"
            "不同类型 Agent 的活跃度与影响力需符合其在验证中的角色（投资人 vs 客户 vs 创始人）。"
        )
