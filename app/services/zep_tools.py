...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## 创业想法验证 · 深度检索分析",
            f"分析问题: {self.query}",
            f"验证场景（模拟需求）: {self.simulation_requirement}",
            f"\n### 模拟数据统计",
            f"- 相关模拟事实: {self.total_facts}条",
...
默认模拟场景为「创业想法验证」：Agent 可能包括投资人、同行创始人、目标客户、犹豫用户、竞品关注者等。

要求：
1. 每个子问题应该足够具体，可以在模拟世界中找到相关的Agent行为或事件
2. 子问题应覆盖验证维度：付费/采用意愿、投资兴趣、竞争与差异化、风险与反对意见、传播与口碑等（按问题相关性选取）
3. 子问题应该与模拟场景相关
...
返回JSON格式的子问题列表。"""
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            "你正在接受一次与「创业想法验证」相关的采访。请结合你的人设、所有的过往记忆与行动，"
            "以纯文本方式直接回答以下问题（保持人设：投资人/创始人/客户/犹豫者等）。\n"
            "回复要求：\n"
            ...
            "5. 每个问题的回答之间用空行分隔\n"
            "6. 回答要有实质内容，每个问题至少回答2-3句话；若你是投资人请明确投资态度线索，若你是客户请明确购买意愿线索\n\n"
        )
...
在「创业想法验证」场景下，应尽量覆盖：至少一类投资人视角（若列表中存在）、至少一类创始人/同行视角、至少一类目标客户或潜在买家视角；可包含犹豫或反对者以反映真实张力。

选择标准：
1. Agent的身份/职业与采访主题相关
2. Agent可能持有独特或有价值的观点（投资、购买、质疑）
3. 选择多样化的视角（看好/不看好/犹豫/专业挑剔等）
4. 优先选择与「该创业想法」讨论直接相关的角色
...
默认用于「创业想法验证」：问题应帮助判断产品/商业模式是否值得投资、是否有人愿意付费、主要风险是什么。

问题要求：
1. 开放性问题，鼓励详细回答
2. 针对不同角色（投资人/创始人/客户）可有不同侧重
3. 涵盖动机、付费意愿、竞品、风险、增长假设等维度（按主题选3-5个）
4. 语言自然，像真实采访一样
5. 每个问题控制在50字以内，简洁明了
6. 直接提问，不要包含背景说明或前缀
...
摘要应服务于「创业想法验证」：点明对用户侧（买不买）、投资侧（投不投）、创始人 peer 侧（靠不靠谱）的信号；若负面信号强于正面，须如实体现。

摘要要求：
1. 提炼各方主要观点
2. 指出观点的共识和分歧（尤其是「验证通过 vs 不通过」的证据）
3. 突出有价值的引言
4. 客观中立，不偏袒任何一方；不得粉饰失败信号
5. 控制在1000字内
