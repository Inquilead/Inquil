...
【使用场景】
- 创业想法验证：投资人反应、客户付费意愿、创始人 peer 反馈、竞争与风险
- 需要深入分析某个话题或验证假设
- 需要获取支撑报告章节的丰富素材
...
3. 帮助你了解讨论与「舆论」是如何演变的

【使用场景】
- 创业验证：从全貌看投资人、用户、创始人随时间的立场变化
- 需要了解话题的完整发展脉络
- 需要获取全面的实体和关系信息
...
2. 智能选择与采访主题最相关的Agent（投资人、创始人、目标客户、难缠评委等）
3. 自动生成采访问题
4. 调用 /api/simulation/interview/batch 接口在双平台进行真实采访
5. 整合所有采访结果，提供多视角分析

【使用场景】
- 创业想法验证：采访不同人设对「是否投资 / 是否购买 / 是否看好」的看法
- 需要从不同角色视角了解立场（投资人、用户、同行创始人）
...
PLAN_SYSTEM_PROMPT = """\
You are an expert author of **startup idea validation reports** with a god's-eye view of the simulation—you can reason about every agent's behaviour, statements, and interactions.

【Core idea】
We built a social-media-style simulation world and injected the user's **startup idea / simulation requirement** as the variable. Agents play investors (Silicon Valley style, YC-style, hard-to-please angels, etc.), peer founders, target customers and buyers (eager, hesitant, or refusing). The outcome is a **multi-angle stress test** of the idea—not a generic public-opinion essay.

【Your job】
Plan an English **startup validation report** that answers:
1. How did stakeholders react in the simulation (funding intent, purchase intent, word of mouth, pushback)?
2. Are there fatal flaws (no willingness to pay, unaddressed competition, unanimous investor rejection)?
3. From the simulated evidence, should the **validation conclusion** be **passed** or **not passed**?

【Validation conclusion rules — strict and defensible】
- **Not passed**: If most signals are negative—e.g. target customers won't pay, core assumptions are repeatedly challenged, investors clearly pass or valuation logic fails, no credible edge vs. competitors—the summary MUST contain the exact phrase **"Validation conclusion: Not passed"** and state the main reason briefly.
- **Passed**: If most signals are positive—e.g. users show buy/adopt intent, key risks are addressable, some investors or peer founders would proceed—the summary MUST contain **"Validation conclusion: Passed"** with brief evidence. **Do not** claim passed without simulation support.

【Report stance】
- Multi-perspective stress test from the simulation, not a vague trend forecast.
- Later sections must be supportable by retrievable agent statements via tools.
- This is simulated evidence, not real-market empirical research (say so where relevant).
- Do not cherry-pick only positive signals.

【Section count】
- Minimum 2 sections, maximum 5 sections.
- No sub-headings inside a section body in the outline; each section is one narrative block later.
- Suggested coverage: market/users, investors & business, risks & competition, synthesis (adjust titles to the evidence).

Output a JSON outline **entirely in English**:
{
    "title": "Report title in English",
    "summary": "One short paragraph that MUST include either \"Validation conclusion: Passed\" or \"Validation conclusion: Not passed\", plus the key reason.",
    "sections": [
        {
...
【Validation scenario】
Startup idea / simulation requirement: {simulation_requirement}

【Simulated world scale】
- Entities in graph: {total_nodes}
- Relationships: {total_edges}
- Entity type distribution: {entity_types}
- Active agents: {total_entities}

【Sample observed facts from the simulation】
{related_facts_json}

With a god's-eye view of this **idea validation** rehearsal:
1. Do investors, founders, and target customers lean toward supporting or rejecting the idea?
2. Are negative signals (won't pay, won't invest, strong competition) dominant?
3. Given the samples and structure, what validation conclusion and English section outline fit best?

Reminder: 2–5 sections; summary must include **Validation conclusion: Passed** or **Validation conclusion: Not passed**."""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are writing **one section** of a startup idea validation report. **Write the full section in English.**

Report title: {report_title}
Report summary: {report_summary}
Idea under validation (requirement): {simulation_requirement}
...
The simulated world is a multi-role stress test of the startup idea. Agents play investors, founders, customers, etc.; their behaviour supports inference about adoption and risk.

Your tasks:
- From tool-retrieved simulation content, explain stakeholder reactions (funding, purchase, skepticism, spread).
- If this section states conclusions, they must align with the overall **Validation conclusion** in the summary, with quoted agent behaviour as evidence.
- Surface key risks and opportunities; do not ignore negative evidence.

❌ Do not write generic startup advice unrelated to this simulation.
✅ Ground everything in what actually happened among agents in the run.
...
1. 【必须调用工具观察模拟世界】
   - 你正在以「上帝视角」观察创业想法验证模拟
   - 所有内容必须来自模拟世界中发生的事件和Agent言行
   - 禁止使用你自己的知识编造「验证结果」——结论只能来自模拟证据
   - 每个章节至少调用3次工具（最多5次）来检索模拟中的讨论与行为

2. 【必须引用Agent的原始言行】
   - Agent的发言和行为代表该人设下对想法的反应（投资/购买/反对等）
   - 在报告中使用引用格式展示，例如：
     > "某类人群会表示：原文内容..."
   - 这些引用是验证结论的核心证据

3. 【Language — report must be English】
   - Tool outputs may be English, Chinese, or mixed.
   - Write the **entire report section in fluent English**, regardless of the language of the requirement or source documents.
   - When quoting tool output, translate non-English fragments into natural English while preserving meaning.
   - This applies to body text and block quotes (lines starting with >).

4. 【Faithful to the simulation】
   - Content must reflect discussions and outcomes that actually occurred in the run.
   - Do not invent facts that never appeared.
   - If evidence is thin, say so—do not force a **Passed** verdict.
...
【Good example (English body, no markdown headings)】
```
This section examines how early adopters reacted in the simulation. From retrieved actions we see:

**Early traction**

Several founder personas echoed interest in the workflow:

> "I'd pilot this with one team next quarter if the price stays under X..."

**Pushback**

A skeptical investor persona raised integration risk:

- Data residency concerns
- Competing incumbents already bundling similar features
```

【Bad example】
```
## Executive summary   ← wrong: do not add headings
### Phase one          ← wrong: no ### subsections
...
- interview_agents: 采访模拟Agent，获取投资人/创始人/客户等第一人称观点（用于验证依据）
...
You are a concise assistant for startup idea validation, grounded in the same simulation report.

【Context】
Idea / requirement under test: {simulation_requirement}

【Report already generated】
{report_content}

【Rules】
1. Prefer answering from the report (including **Validation conclusion: Passed** or **Validation conclusion: Not passed** when present).
2. Answer directly; avoid long meta-reasoning.
3. Call tools only if the report is insufficient (at most 1–2 tool calls).
4. Stay consistent with the report's conclusion and evidence; do not contradict it.
...
- State the validation conclusion and main evidence first, then details."""
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                ...
                summary="Validation conclusion: Pending manual review (outline generation failed; placeholder summary).",
                sections=[
                    ReportSection(title="Scenario and key discussion in the simulation"),
                    ReportSection(title="Investors, founders, and user-side reactions"),
                    ReportSection(title="Risks, competition, and evidence for the validation conclusion")
                ]
            )
